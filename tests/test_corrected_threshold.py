...
def test_corrected_threshold():
    ...
    # Load metadata
    metadata_file = "/home/gg/Code/db_samples/tmp/metadata_84C10-AD_23E10-DBD_Suvar3_9_SD_0801.csv"
    ...
    animals_018 = metadata_linked[metadata_linked['machine_name'] == 'ETHOSCOPE_018'].head(2)
    animals_072 = metadata_linked[metadata_linked['machine_name'] == 'ETHOSCOPE_072'].head(2)
    ...
    def load_with_threshold(animals, threshold, machine_name):
        ...
        def custom_motion_detector(data, time_window_length, masking_duration=6, velocity_correction_coef=3e-3):
    ...
    for animals, threshold, label in configs:
        ...
        if data is not None and len(data) > 0:
            ...
            print(f"  Max velocity - Mean: {max_vel_mean:.3f}, Median: {max_vel_median:.3f}, Std: {max_vel_std:.3f}")
        else:
    ...
    if len(results) >= 3:
        ...
        improvement = corrected_018['sleep_percentage'] - original_018['sleep_percentage']
        similarity_to_072 = abs(corrected_018['sleep_percentage'] - reference_072['sleep_percentage'])
        ...
        original_diff = abs(original_018['sleep_percentage'] - reference_072['sleep_percentage'])
        corrected_diff = abs(corrected_018['sleep_percentage'] - reference_072['sleep_percentage'])
        
        if corrected_diff < original_diff:
            print(f"  ✓ Corrected threshold brings ETHOSCOPE_018 closer to ETHOSCOPE_072 behavior")
        else:
...
def create_comparison_plots(results):
    ...
    bars = axes[0,0].bar(range(len(labels)), sleep_values, 
    ...
    axes[0,0].set_xticklabels([label.split('(')[0].strip() for label in labels], rotation=45)
    ...
    for i, (label, result) in enumerate(results.items()):
        if 'data' in result:
            axes[0,1].hist(result['data']['max_velocity'], bins=30, alpha=0.6, 
                          label=label.split('(')[0].strip(), color=colors[i], density=True)
    ...
    bars = axes[1,0].bar(range(len(labels)), movement_values,
                        color=['red', 'green', 'blue'][:len(labels)])
    ...
    axes[1,0].set_xticklabels([label.split('(')[0].strip() for label in labels], rotation=45)
