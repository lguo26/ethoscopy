...
def test_velocity_thresholds():
    ...
    animals_018 = metadata_linked[metadata_linked['machine_name'] == 'ETHOSCOPE_018'].head(1)
    animals_072 = metadata_linked[metadata_linked['machine_name'] == 'ETHOSCOPE_072'].head(1)
    ...
    for animals, machine_name, fps in [(animals_018, 'ETHOSCOPE_018', 4.05), 
        ...
        for vel_threshold in velocity_thresholds:
            try:
                ...
                if data is not None and len(data) > 0:
                    ...
                    print(f"  Threshold {vel_threshold:4.1f}: Sleep {sleep_pct:5.1f}% | Movement {moving_pct:5.1f}%")
    ...
    for machine in ['ETHOSCOPE_018', 'ETHOSCOPE_072']:
        ...
        if len(machine_results) > 0:
            target_sleep = 30.0
            machine_results['sleep_diff'] = abs(machine_results['sleep_percentage'] - target_sleep)
